...
def get_real_data_ISIC(path):
    ...
    real_images_l = [img.split('/')[-1].replace('.jpg','') for img in Tr_list]
    return real_images_l
    
def split_data_ISIC(dataset):
    Train_img = dataset[0:1815]
    Validation_img = dataset[1815:1815+259]
    Test_img = dataset[1815+259:2594]
    print("Number of ISIC training images:", len(Train_img))
    print("Number of ISIC validation images:", len(Validation_img))
    print("Number of ISIC test images:", len(Test_img))
...
def image_and_mask_path_ISIC(dataset, data_dir_real):
    dataset_image = []
    dataset_mask = []
    for image_ID in dataset:
        if os.path.exists(image_ID): #synthetic
            ...
            nameimage_path = os.path.join(data_dir_real +  'ISIC2018_Task1-2_Training_Input/' + image_ID +'.jpg')
            dataset_image.append(nameimage_path)
            mask_path = os.path.join(data_dir_real +  'ISIC2018_Task1_Training_GroundTruth/' + image_ID + '_segmentation' +'.png')
            dataset_mask.append(mask_path)
...
real_data_list = get_real_data_ISIC(real_data_path)
dataset_real_train, dataset_real_val, dataset_real_test = split_data_ISIC(real_data_list)
...
if real_data_ratio != 1:
    ...
    print("Replacing real with synthetic data ...")
...
train_images, train_masks = image_and_mask_path_ISIC(combined_dataset_train, real_data_path)
val_images, val_masks = image_and_mask_path_ISIC(combined_dataset_val, real_data_path)
test_images, test_masks = image_and_mask_path_ISIC(combined_dataset_test, real_data_path)
