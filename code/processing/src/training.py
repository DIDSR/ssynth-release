...
from train_validate import train, validate, test
...
print('run_id ' + config["run"]["run_id"].zfill(2))

logger = get_logger(filename=f"{config['model']['name']}", dir=f"{config['run']['log']}/{config['dataset']['name']}")
print_config(config, logger)

print('run_id ' + config["run"]["run_id"].zfill(2))
# create the writer for tensorboard
writer = SummaryWriter(f'{config["run"]["writer_dir"]}/{config["run"]["name"]}/{config["run"]["run_id"].zfill(2)}/{config["model"]["name"]}/')
print('tensorboard ' + f'{config["run"]["writer_dir"]}/{config["run"]["name"]}/{config["run"]["run_id"].zfill(2)}/{config["model"]["name"]}/')
...
model_path = get_model_path(name=ID, config=config)
...
# tr_dataloader, vl_dataloader, vl2real_dataloader, vl2synth_dataloader = get_dataloaders(config, ["tr", "vl","vl2real","vl2synth"])
...
for epoch in range(start_epoch, epochs):
    ...
    writer.add_scalars(
        f"Loss/{config['training']['loss_name'].upper()}",
